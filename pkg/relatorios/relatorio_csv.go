...
import (
	...
	svccontabil "github.com/dude333/rapinav2/pkg/contabil/servico"
...
func NovoCSV(db *sqlx.DB) (*CSV, error) {
	svc, err := svccontabil.NovoDemonstraçãoFinanceira(db, "")
	if err != nil {
		return &CSV{}, err
	}
...
func Csv(db *sqlx.DB, cnpj string, ano int) error {
	c, err := NovoCSV(db)
	if err != nil {
		return err
	}
